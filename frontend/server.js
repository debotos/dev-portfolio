...
if (process.env.NODE_ENV === 'production') {
	...
	HOST_URL = 'https://deb-portfolio-cms.herokuapp.com'
...
console.log(
	process.env.MAILER_PASSWORD ? '' : 'Put your your mail & password for development testing!'
)
...
app.get('/', function(req, res) {
	try {
		const getProfileData = () => axios.get(`${HOST_URL}/api/profile/user/${EMAIL}`)
		const getPortfolioData = () => axios.get(`${HOST_URL}/api/portfolio/user/${EMAIL}`)
		...
		const allResponse = axios
			...
			.catch(ex => {
				console.log(`Error (Maybe API service ${HOST_URL} crashed!) => `, ex.response.data)
